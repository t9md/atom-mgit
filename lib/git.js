...
module.exports = class Git {
  ...
  async commit(messageFilePath) {
    console.log("commit", messageFilePath)
    return this.runGitCommand(`commit --cleanup=strip --file=${messageFilePath}`)
  }

  async push(remote, branch) {
    const args = `-c push.default=simple push ${remote} ${branch} --porcelain`
