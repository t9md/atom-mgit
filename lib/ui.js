...
module.exports = class Ui {
  ...
  async start() {
    ...
    if (this.gitStatus.isEmpty()) {
      this.editor.setText("## Clean")
    } else {
      this.editor.setText("## " + this.statusMessage + "\n")
      this.gitStatus.renderFromRow(1)
    }
  ...
  async diff() {
    ...
    const item = this.gitStatus.getItemForRow(row)
    if (!item) return
    ...
    this.diffEditor.renderForFile(item.fileName, this.phase === "commit")
  }
  ...
  async startCommit() {
    if (!this.gitStatus.hasStagedItem()) {
