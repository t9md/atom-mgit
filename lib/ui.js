...
module.exports = class Ui {
  ...
  registerCommands() {
    return atom.commands.add(this.editor.element, {
      ...
      "mgit-ui:open-diff-and-focus": () => this.openDiffAndFocus(),
    ...
    return atom.workspace.observeTextEditors(editor => {
      ...
      this.disposables.add(
        editor.onDidSave(async event => {
          ...
          if (this.diffEditor) {
            await this.diffEditor.refreshIfBasedOnEditor(editor)
          }
        })
      )
    })
  ...
  async start() {
    ...
    this.editor.setCursorBufferPosition([1, 0])
    if (this.needUpdateDiff()) this.diff()
  ...
  async openDiffAndFocus() {
    await this.diff()
    this.diffEditor.activate()
  }
