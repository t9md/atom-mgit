...
const {findInEditor, getRepositoryForEditor} = require("./utils")
...
module.exports = class Ui {
  ...
  observeEditors() {
    // const refreshStatus = this.refreshStatus.bind(this)
  ...
  async refreshDiffIfNecessary(editor) {
    if (this.fileNameForDiff && this.fileNameForDiff === atom.project.relativize(editor.getPath())) {
      const text = (await this.git.diff(this.fileNameForDiff)) || "no diff"
  ...
  moveToNextHunk() {
    const from = this.diffEditor.getCursorBufferPosition()
    const regex = /^[+-]/g
    let rowsToSkip = isHunkRow(this.diffEditor, from.row) ? from.row + 1 : -1
    const point = findInEditor(this.diffEditor, regex, "forward", {from}, event => {
    ...
    if (point) this.diffEditor.setCursorBufferPosition(point)
  }
  ...
  moveToPreviousHunk() {
    ...
    const regex = /^[+-]/g
    let rowsToSkip = isHunkRow(this.diffEditor, from.row) ? from.row - 1 : -1
    const point = findInEditor(this.diffEditor, regex, "backward", {from}, event => {
      const {start} = event.range
      if (start.row === rowsToSkip) {
        rowsToSkip--
        return
